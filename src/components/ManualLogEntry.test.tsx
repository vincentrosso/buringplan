...
describe('ManualLogEntry', () => {
  ...
  it('submits computed fields, calls onSaved, and resets to the collapsed state', async () => {
    const user = userEvent.setup();
    const onSaved = vi.fn();
    ...
    await user.type(screen.getByLabelText('Avg speed (mph)'), '55');
    ...
    expect(saved.avgSpeedMph).toBe(55);
  ...
  it('treats blank optional fields as zero', async () => {
    const user = userEvent.setup();
    render(<ManualLogEntry onSaved={vi.fn()} />);
    await user.click(screen.getByText('+ Add manual entry'));
